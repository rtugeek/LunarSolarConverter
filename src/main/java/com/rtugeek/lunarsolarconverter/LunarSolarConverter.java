...
public class LunarSolarConverter {
    ...
    private static Solar SolarFromInt(long g) {
        ...
        solar.year = (int) y;
        solar.monthOfYear = (int) mm;
        solar.dayOfMonth = (int) dd;
        return solar;
    }

    public static int getLeap(Lunar lunar) {
        int days = lunar_month_days[lunar.year - lunar_month_days[0]];
        return GetBitInt(days, 4, 13);
    }
    ...
    public static Solar LunarToSolar(Lunar lunar) {
        ...
        int loopend = leap;
        if (!lunar.isLeap) {
            if (lunar.monthOfYear <= leap || leap == 0) {
                loopend = lunar.monthOfYear - 1;
            } else {
                loopend = lunar.monthOfYear;
            }
        }
        for (int i = 0; i < loopend; i++) {
            offset += GetBitInt(days, 1, 12 - i) == 1 ? 30 : 29;
        }
        offset += lunar.dayOfMonth;
    ...
    public static Lunar SolarToLunar(Solar solar) {
        ...
        int data = (solar.year << 9) | (solar.monthOfYear << 5)
                | (solar.dayOfMonth);
        int solar11 = 0;
        ...
        int m = GetBitInt(solar11, 4, 5);
        int d = GetBitInt(solar11, 5, 0);
        long offset = SolarToInt(solar.year, solar.monthOfYear,
                solar.dayOfMonth) - SolarToInt(y, m, d);
        ...
        lunarD = (int) (offset);
        lunar.year = lunarY;
        lunar.monthOfYear = lunarM;
        lunar.isLeap = false;
        if (leap != 0 && lunarM > leap) {
            lunar.monthOfYear = lunarM - 1;
            if (lunarM == leap + 1) {
                lunar.isLeap = true;
            }
        }
